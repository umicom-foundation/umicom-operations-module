...
int main(void)
{
    ...
    assert(strcmp(adoption->application_id, "org.umicom.operations") == 0);
    assert(umi_product_application_adoption_validate(adoption) ==
           UMI_STATUS_OK);
    assert(umi_operations_productisation_snapshot(&snapshot) ==
           UMI_STATUS_OK);
    ...
    assert(umi_operations_product_session_init(&session) ==
           UMI_STATUS_OK);
    ...
    assert(umi_operations_product_workspace_guide(&workspace_guide) ==
           UMI_STATUS_OK);
